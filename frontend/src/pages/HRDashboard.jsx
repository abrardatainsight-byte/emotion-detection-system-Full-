...
// ─── Global Pulse tab ─────────────────────────────────────────────────────────
function GlobalPulseTab({ range, company }) {
  const [pulse, setPulse] = useState(null);
  const [analyzing, setAnalyzing] = useState(false);

  const load = useCallback(async () => {
    try { setPulse(await getGlobalPulse(range, company)); } catch {}
  }, [range, company]);
...
// ─── Macro Analytics tab ──────────────────────────────────────────────────────
function MacroAnalyticsTab({ range, company }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [d, t, i, m] = await Promise.all([
        getDistribution(range, company),
        getWeeklyTrend(company),
        getIntensity(company),
        getMatrix(range, company),
      ]);
...
// ─── Insight Hub tab ──────────────────────────────────────────────────────────
function InsightHubTab({ range, company }) {
  ...
  const load = useCallback(async () => {
    try { setData(await getEmployees(range, debouncedSearch, company)); } catch {}
  }, [range, debouncedSearch, company]);
...
export default function HRDashboard() {
  ...
  const company = localStorage.getItem("company") || "EYEAI"; // Safely grab company from login
      ...
      <div style={{ display: "flex", height: "100vh", background: "#F8FAFC", fontFamily: '"Inter", system-ui, sans-serif' }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflowY: "auto" }}>
          ...
          <div style={{ padding: "36px 44px 20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            <div>
              <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#94A3B8" }}>Lumora · {company} Workspace · {username}</p>
          ...
          <div style={{ padding: "0 44px 44px" }}>
            {tab === "Global Pulse" && <GlobalPulseTab range={range} company={company} />}
            {tab === "Macro Analytics" && <MacroAnalyticsTab range={range} company={company} />}
            {tab === "Insight Hub" && <InsightHubTab range={range} company={company} />}
          </div>
        </div>
      </div>
    </>
